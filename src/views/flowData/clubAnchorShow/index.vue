	...
	<el-card shadow="hover">
		<template #header>
			<div class="header">
				...
				<el-button type="warning" :loading="syncing" @click="runSync">同步俱乐部主播开播数据</el-button>
			</div>
		</template>
...
import { getClubAnchorShowList, getClubAnchorShowProgress, getClubAnchorShowSummary, syncClubAnchorShow } from '/@/api/system/flowData';
import VVSyncProgress from '/@/components/vvSyncProgress/index.vue';

defineOptions({ name: 'flowDataClubAnchorShow' });
...
const runSync = async () => {
	syncing.value = true;
	try {
		const response: any = await syncClubAnchorShow();
