	...
	<el-card shadow="hover">
		<template #header>
			<div class="header">
				...
				<el-button type="warning" :loading="syncing" @click="runSync">同步主播开播数据</el-button>
			</div>
		</template>
		<VVSyncProgress sync-type="ANCHOR_SHOW" :active="syncing" :loader="getAnchorShowProgress" />
		...
		<el-table v-loading="table.loading" :data="table.list" border stripe>
			...
			<el-table-column prop="roomName" label="频道昵称" min-width="130" />
			<el-table-column prop="cuteNumber" label="靓号ID" width="110" />
			<el-table-column prop="totalFlow" label="流水(元)" width="120" />
			<el-table-column prop="totalTimeStr" label="累计麦位排序" width="130" />
...
import { getAnchorActivityHallOptions, getAnchorShowList, getAnchorShowProgress, getAnchorShowSummary, syncAnchorShow } from '/@/api/system/flowData';
import VVSyncProgress from '/@/components/vvSyncProgress/index.vue';

defineOptions({ name: 'flowDataAnchorShow' });
...
const load = async () => {
	...
	try {
		...
		const response: any = await getAnchorShowList({ ...query, startDate, endDate });
...
const loadSummary = async () => Object.assign(summary, (await getAnchorShowSummary() as any).data);
...
const runSync = async () => {
	syncing.value = true;
	try {
		const response: any = await syncAnchorShow();
