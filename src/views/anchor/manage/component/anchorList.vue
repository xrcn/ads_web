<template>
	<div><MobileRecordList :data="tableData.data" row-key="profileId" data-mobile-view="anchor-list"><template #desktop><el-table :data="tableData.data" @selection-change="selection=$event">
		...
		<el-table-column label="操作" width="250" fixed="right"><template #default="{row}"><el-button text type="primary" @click="openEditDialog(row)">编辑</el-button><el-button v-if="hasBankCardOwner(row)" text type="primary" @click="openBankCardDialog(row)">工资卡</el-button><el-button v-if="row.recordType==='PROFILE'&&row.recordState==='ACTIVE'" v-auth="'api/v1/system/anchor/profile/batchDelete'" text type="danger" @click="deleteRow(row)">删除</el-button><el-button v-if="row.recordType==='LEGACY_ANCHOR'" v-auth="'api/v1/system/anchor/delete'" text type="danger" @click="deleteRow(row)">删除</el-button><el-button v-if="row.recordType==='PROFILE'&&row.recordState==='ACTIVE'" v-auth="'api/v1/system/anchor/profile/batchIgnore'" text type="warning" @click="runBatch('IGNORE',[row.profileId])">忽略</el-button><el-button v-if="row.recordState==='IGNORED'" v-auth="'api/v1/system/anchor/profile/batchCancelIgnore'" text @click="runBatch('CANCEL_IGNORE',[row.profileId])">取消忽略</el-button></template></el-table-column>
		...
		<div class="mobile-record-card__actions"><el-button type="primary" @click="openEditDialog(row)">编辑</el-button><el-button v-if="hasBankCardOwner(row)" @click="openBankCardDialog(row)">工资卡</el-button><el-dropdown><el-button>更多</el-button><template #dropdown><el-dropdown-menu><el-dropdown-item v-if="row.recordType==='PROFILE'&&row.recordState==='ACTIVE'"><el-button v-auth="'api/v1/system/anchor/profile/batchDelete'" text type="danger" @click="deleteRow(row)">删除</el-button></el-dropdown-item><el-dropdown-item v-if="row.recordType==='LEGACY_ANCHOR'"><el-button v-auth="'api/v1/system/anchor/delete'" text type="danger" @click="deleteRow(row)">删除</el-button></el-dropdown-item><el-dropdown-item v-if="row.recordType==='PROFILE'&&row.recordState==='ACTIVE'"><el-button v-auth="'api/v1/system/anchor/profile/batchIgnore'" text @click="runBatch('IGNORE',[row.profileId])">忽略</el-button></el-dropdown-item><el-dropdown-item v-if="row.recordState==='IGNORED'"><el-button v-auth="'api/v1/system/anchor/profile/batchCancelIgnore'" text @click="runBatch('CANCEL_IGNORE',[row.profileId])">取消忽略</el-button></el-dropdown-item></el-dropdown-menu></template></el-dropdown></div>
...
const hasBankCardOwner=(row:any)=>row.bindings?.some((binding:any)=>binding.anchorId&&binding.hallId);
...
const deleteRow=async(row:any)=>{if(row.recordType==='PROFILE'){await runBatch('DELETE',[row.profileId]);return;}await ElMessageBox.confirm(`确认删除历史主播 ${row.nickname}？存在工资卡时将拒绝删除；已有麦序历史会保留。`,'删除确认',{type:'warning'});await deleteLegacyAnchor(row.id);ElMessage.success('删除成功');loadList();};
