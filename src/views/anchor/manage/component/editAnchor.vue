<template><el-dialog :title="profileMode?'编辑主播档案':(form.id?'编辑主播':'新增主播')" v-model="visible" width="850px">
	<el-form ref="formRef" :model="form" label-width="92px"><el-row :gutter="18">
		...
		<el-col v-if="profileMode" :span="12" class="profile-hall-field"><el-form-item label="所属厅"><el-select v-model="form.hallId" clearable placeholder="请选择所属厅"><el-option v-for="h in formHallOptions" :key="h.hallId" :label="h.hallName" :value="h.hallId" :disabled="h.disabled"/></el-select></el-form-item></el-col>
...
import { computed,reactive,ref } from 'vue'; import { ElMessage,ElMessageBox } from 'element-plus';
...
const withCurrentHall=(active:any[],hallId:any,hallName:string)=>!hallId||active.some((item:any)=>item.hallId===hallId)?active:[...active,{hallId,hallName:`${hallName||hallId}（已停用）`,disabled:true}];
const formHallOptions=computed(()=>withCurrentHall(props.hallOptions,form.hallId,(form as any).hallName));
const bindingHallOptions=(binding:any)=>withCurrentHall(props.hallOptions,binding.hallId,binding.hallName);
...
const addBinding=()=>bindings.value.push({id:0,localKey:`new-${Date.now()}-${bindings.value.length}`,platformCode:'vvxqiu',anchorId:'',hallId:form.hallId||''});
