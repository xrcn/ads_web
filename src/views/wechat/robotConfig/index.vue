				...
				<el-tabs v-model="activeTab" :tab-position="tabPosition" class="config-tabs mobile-config-tabs-collapsed">
					<el-tab-pane v-for="tab in tabs" :key="tab.name" :label="tab.label" :name="tab.name">
						...
						<template v-else-if="tab.name === 'aiChat'">
							<div class="panel-heading"><div><h4>小助手功能</h4><p>聊天、业务查询和受控执行操作分别启用；执行操作只接受精确 @。</p></div><el-button v-if="canSaveAIChat" type="primary" :loading="aiChatSaving" @click="saveAIChatForm">保存本页</el-button></div>
						...
						<template v-else-if="tab.name === 'anchorCustomCommand'">
							<AnchorCustomCommandTab :group-id="selectedGroupId" :last-successful-sync-at="overview?.lastSuccessfulMemberSyncAt" :can-read="canReadAnchorCustomCommands" :can-save="canSaveAnchorCustomCommands" :can-delete="canDeleteAnchorCustomCommand" :can-preview="canPreviewAnchorCustomCommand" :can-sync-members="canSyncMembers" />
						</template>
...
import AnchorCustomCommandTab from './components/AnchorCustomCommandTab.vue';
...
const canReadAnchorCustomCommands=auth('api/v1/system/wechatRobotGroup/anchorCustomCommand/list')&&auth('api/v1/system/wechatRobotGroup/anchorCustomCommand/config');const canSaveAnchorCustomCommands=auth('api/v1/system/wechatRobotGroup/anchorCustomCommand/configSave');const canDeleteAnchorCustomCommand=auth('api/v1/system/wechatRobotGroup/anchorCustomCommand/delete');const canPreviewAnchorCustomCommand=auth('api/v1/system/wechatRobotGroup/anchorCustomCommand/preview');const canSyncMembers=auth('api/v1/system/wechatRobotGroup/memberSync');
...
const aiChatSaving=ref(false);const aiChatConfig=reactive<WechatRobotGroupAIChatConfig>({groupId:0,enabled:0, followupEnabled: 0,businessQueryEnabled:0,memoryEnabled:0,businessAccess:'OPERATORS_ONLY',assistantActionEnabled:0,assistantActionAccess:'OPERATORS_ONLY',robotGroupNickname:'',configurationReady:false,memoryConfigurationReady:false});const loadedAIChatGroupId=ref<number>();let aiChatRequestGeneration=0;
const mobileMedia = window.matchMedia('(max-width: 768px)');
const tabs = [
	{ name: 'basic', label: '基础信息', description: '群、厅号、绑定机器人和启停状态。' },
	...(canReadAIChat ? [{ name: 'aiChat', label: '小助手功能', description: '按群配置聊天、查询和受控执行操作。' }] : []),
	...
	...(canReadAnchorCustomCommands ? [{ name: 'anchorCustomCommand', label: '专属扣排配置', description: '主播表情或图片专属扣排。' }] : []),
...
const loadAIChat=async()=>{if(!selectedGroupId.value||!canReadAIChat)return;const groupId=selectedGroupId.value;const generation=++aiChatRequestGeneration;loadedAIChatGroupId.value=undefined;Object.assign(aiChatConfig,{enabled:0, followupEnabled: 0,businessQueryEnabled:0,memoryEnabled:0,businessAccess:'OPERATORS_ONLY',assistantActionEnabled:0,assistantActionAccess:'OPERATORS_ONLY',robotGroupNickname:'',configurationReady:false,memoryConfigurationReady:false});const res=await getWechatRobotGroupAIChatConfig(groupId);if(generation===aiChatRequestGeneration&&selectedGroupId.value===groupId){const loadedConfig = { ...res.data };if (loadedConfig.enabled !== 1) {loadedConfig.businessQueryEnabled = 0;loadedConfig.followupEnabled = 0;}Object.assign(aiChatConfig, loadedConfig);loadedAIChatGroupId.value=groupId;}};
const saveAIChatForm=async()=>{const groupId=selectedGroupId.value;if(!groupId||!canSaveAIChat)return;if(loadedAIChatGroupId.value!==groupId){ElMessage.error('当前群配置尚未加载完成');return;}if(aiChatConfig.enabled===1&&!aiChatConfig.configurationReady){ElMessage.error('模型配置尚未就绪');return;}if(aiChatConfig.memoryEnabled===1&&!aiChatConfig.memoryConfigurationReady){ElMessage.error('记忆加密配置尚未就绪');return;}if(aiChatConfig.businessQueryEnabled===1&&aiChatConfig.enabled!==1){ElMessage.error('请先开启小助手聊天');return;}const enabled=aiChatConfig.enabled;const followupEnabled=enabled===1?aiChatConfig.followupEnabled:0;const businessQueryEnabled=aiChatConfig.businessQueryEnabled;const memoryEnabled=aiChatConfig.memoryEnabled;const businessAccess=aiChatConfig.businessAccess;const assistantActionEnabled=aiChatConfig.assistantActionEnabled;const assistantActionAccess=aiChatConfig.assistantActionAccess;const generation=++aiChatRequestGeneration;aiChatSaving.value=true;try{const res=await saveWechatRobotGroupAIChatConfig({groupId,enabled,followupEnabled,businessQueryEnabled,memoryEnabled,businessAccess,assistantActionEnabled,assistantActionAccess});if(generation===aiChatRequestGeneration&&selectedGroupId.value===groupId){Object.assign(aiChatConfig,res.data);loadedAIChatGroupId.value=groupId;ElMessage.success('小助手功能配置已保存');}}finally{aiChatSaving.value=false;}};
...
watch(selectedGroupId,()=>{
	...
	Object.assign(aiChatConfig,{enabled:0, followupEnabled: 0,businessQueryEnabled:0,memoryEnabled:0,businessAccess:'OPERATORS_ONLY',assistantActionEnabled:0,assistantActionAccess:'OPERATORS_ONLY',robotGroupNickname:'',configurationReady:false,memoryConfigurationReady:false});
