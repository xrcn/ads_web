...
import { batchClearWechatRobotGroupSchedulePlan, batchSaveWechatRobotGroupSchedulePlan, cancelWechatRobotGroupSpecialTop, closeWechatRobotGroupReport, getWechatRobotGroupActiveReports, getWechatRobotGroupConfigOverview, getWechatRobotGroupList, getWechatRobotGroupQueueRules, getWechatRobotGroupReportConfig, getWechatRobotGroupReportConfigAudit, getWechatRobotGroupScheduleExceptions, getWechatRobotGroupSchedulePlan, getWechatRobotGroupSchedulePlanAudit, getWechatRobotGroupScheduleTiming, getWechatRobotGroupSpecialTopList, getWechatRobotGroupStatisticsConfig, getWechatRobotGroupStatisticsConfigAudit, getWechatRobotGroupTemplateCommandAudit, getWechatRobotGroupTemplateCommands, grantWechatRobotGroupSpecialTop, resetWechatRobotGroupTemplateCommand, restoreWechatRobotGroupFixedException, restoreWechatRobotGroupHostException, saveWechatRobotGroupFixedException, saveWechatRobotGroupHostException, saveWechatRobotGroupQueueRules, saveWechatRobotGroupReportConfig, saveWechatRobotGroupSchedulePlan, saveWechatRobotGroupScheduleTiming, saveWechatRobotGroupStatisticsConfig, saveWechatRobotGroupTemplateCommand } from '/@/api/wechatRobotGroup';
...
const loadTemplateCommands=async()=>{if(!selectedGroupId.value||!canReadTemplates)return;const res:any=await getWechatRobotGroupTemplateCommands(selectedGroupId.value);templateCommands.value=(res.data.list||[]).map((row:any)=>({...row,aliasInput:(row.groupAliases||[]).join('\n'),scenarios:(row.scenarios||[]).map((scenario:any)=>({...scenario,groupContent:scenario.groupContent||scenario.publicContent}))}));};const delegationModeLabel=(mode:string)=>mode==='ALL'?'所有人':mode==='ADMIN_ONLY'?'仅管理员':mode==='DISABLED'?'禁止代发':'不适用';const saveTemplateCommand=async(row:any)=>{await saveWechatRobotGroupTemplateCommand({groupId:selectedGroupId.value,commandKey:row.commandKey,aliases:row.aliasInput.split(/\r?\n/).map((value:string)=>value.trim()).filter(Boolean),delegationMode:row.delegationMode,scenarios:row.scenarios.map((scenario:any)=>({eventKey:scenario.eventKey,state:scenario.state,content:scenario.groupContent}))});ElMessage.success('模板与口令配置已保存');await loadTemplateCommands();};const resetTemplateCommand=async(row:any)=>{await ElMessageBox.confirm(`确认将“${row.commandName||row.commandKey}”恢复公共默认？群私有模板、口令和代发覆盖都会删除。`,'恢复公共默认',{type:'warning'});await resetWechatRobotGroupTemplateCommand({groupId:selectedGroupId.value,commandKey:row.commandKey});ElMessage.success('已恢复公共默认');await loadTemplateCommands();};const openTemplateAudit=async()=>{const res:any=await getWechatRobotGroupTemplateCommandAudit(selectedGroupId.value!);templateAudits.value=res.data.list||[];templateAuditDrawer.value=true;};
watch(activeTab,(value)=>{if(value==='queue')loadQueueTab();if(value==='timing')loadTiming();if(value==='schedule')loadSchedulePlan();if(value==='report')loadReport();if(value==='checkin')loadStatistics();if(value==='template')loadTemplateCommands();});watch(selectedGroupId,()=>{if(activeTab.value==='queue')loadQueueTab();if(activeTab.value==='timing')loadTiming();if(activeTab.value==='schedule')loadSchedulePlan();if(activeTab.value==='report')loadReport();if(activeTab.value==='checkin')loadStatistics();if(activeTab.value==='template')loadTemplateCommands();});
