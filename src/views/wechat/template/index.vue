	...
	<div class="wechat-message-template-container">
		<el-card shadow="hover">
			...
			<template #default="{ row }">
				...
				<details class="mobile-record-card__details"><summary>查看完整信息</summary><dl class="mobile-record-card__fields"><div><dt>更新时间</dt><dd>{{ row.updatedAt || '-' }}</dd></div><div><dt>触发规则</dt><dd>{{ row.triggerDescription || '-' }}</dd></div></dl><div v-for="scenario in row.scenarios || []" :key="scenario.eventKey" class="member-mobile-card"><strong>{{ scenarioLabel(scenario) }}</strong><dl class="mobile-record-card__fields"><div><dt>事件编码</dt><dd>{{ scenario.eventKey }}</dd></div><div><dt>触发规则</dt><dd>{{ scenario.triggerDescription || '-' }}</dd></div><div><dt>回复模板</dt><dd>{{ scenario.content || '-' }}</dd></div><div><dt>状态</dt><dd>{{ scenario.status === 1 ? '启用' : '停用' }}</dd></div><div><dt>更新时间</dt><dd>{{ scenario.updatedAt || '-' }}</dd></div></dl></div></details>
