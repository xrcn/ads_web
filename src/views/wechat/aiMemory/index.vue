	...
	<div class="wechat-ai-memory-container">
		...
		<el-dialog v-model="formVisible" :title="form.id?'编辑记忆':'新增记忆'" width="640px" destroy-on-close>
			<el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
				...
				<el-form-item label="范围" prop="scopeType"><el-radio-group v-model="form.scopeType" @change="changeMemoryScope"><el-radio value="GROUP">群级</el-radio><el-radio value="GLOBAL" :disabled="form.memoryType==='CONVERSATION'">全局</el-radio></el-radio-group></el-form-item>
				<el-form-item v-if="form.scopeType==='GROUP'||form.memoryType==='PERSON'" :label="form.scopeType==='GLOBAL'?'成员所在群':'微信群'" prop="groupId"><el-select v-model="form.groupId" filterable class="w100" @change="changeMemoryGroup"><el-option v-for="group in groups" :key="group.id" :label="group.groupName" :value="group.id" /></el-select></el-form-item>
				...
				<el-form-item label="敏感级别" prop="sensitivity"><el-radio-group v-model="form.sensitivity"><el-radio value="NORMAL">普通</el-radio><el-radio value="SENSITIVE" :disabled="form.memoryType!=='PERSON'||form.scopeType==='GLOBAL'">敏感</el-radio></el-radio-group></el-form-item>
...
const changeMemoryType=()=>{if(form.memoryType!=='PERSON'){form.subjectWxid='';form.sensitivity='NORMAL';}if(form.memoryType==='CONVERSATION'&&form.scopeType==='GLOBAL')form.scopeType='GROUP';if(form.scopeType==='GLOBAL'&&form.memoryType!=='PERSON'){form.groupId='';void loadMembers(0);}else if(form.groupId)void loadMembers(Number(form.groupId));};
const changeMemoryScope=()=>{if(form.scopeType==='GLOBAL')form.sensitivity='NORMAL';if(form.scopeType==='GLOBAL'&&form.memoryType!=='PERSON'){form.groupId='';form.subjectWxid='';void loadMembers(0);}else if(form.groupId)void loadMembers(Number(form.groupId));};
...
const openEdit=async(row:any)=>{const groupId=row.subjectGroupId||row.groupId||'';Object.assign(form,{id:row.id,memoryType:row.memoryType,scopeType:row.scopeType,groupId,subjectWxid:row.subjectWxid||'',topicKey:row.topicKey,tagsText:row.tags||'',value:row.sensitivity==='SENSITIVE'?'':row.displayValue,sensitivity:row.sensitivity,reason:''});if(groupId)await loadMembers(groupId);formVisible.value=true;};
const payload=()=>({id:form.id||undefined,memoryType:form.memoryType,scopeType:form.scopeType,groupId:form.scopeType==='GROUP'||form.memoryType==='PERSON'?form.groupId||undefined:undefined,subjectWxid:form.memoryType==='PERSON'?form.subjectWxid:undefined,topicKey:form.topicKey,tags:form.tagsText.split(/[，,|]/).map(v=>v.trim()).filter(Boolean),value:form.value,sensitivity:form.sensitivity,reason:form.reason});
