	...
	<div class="system-menu-container">
        ...
        <el-form :inline="true">
          ...
          <el-form-item>
            ...
            <el-button size="default" type="success" class="ml10"  plain @click="onOpenAddMenu(null)" v-auth="'api/v1/system/menu/add'">
              <el-icon>
                <ele-FolderAdd />
              </el-icon>
              新增菜单
        ...
        <el-table-column prop="isHide" label="显示状态" :formatter="formatIsHide" width="120"></el-table-column>
			...
			</el-table></template><template #default="{ row }"><div class="mobile-record-card__header"><h3 class="mobile-record-card__title">{{ row.title || '-' }}</h3><el-tag>{{ row.menuType === 0 ? '目录' : row.menuType === 1 ? '菜单' : '按钮' }}</el-tag></div><dl class="mobile-record-card__fields"><div><dt>路由路径</dt><dd>{{ row.path || '-' }}</dd></div><div><dt>组件路径</dt><dd>{{ row.component || '-' }}</dd></div><div><dt>API 接口</dt><dd>{{ row.name || '-' }}</dd></div></dl><details class="mobile-record-card__details"><summary>查看完整信息</summary><dl class="mobile-record-card__fields"><div><dt>排序</dt><dd>{{ row.weigh ?? 0 }}</dd></div><div><dt>显示状态</dt><dd>{{ formatIsHide(row) }}</dd></div></dl></details><div class="mobile-record-card__actions"><el-button v-if="row.menuType !== 2" v-auth="'api/v1/system/menu/add'" type="primary" @click="onOpenAddMenu(row)">新增</el-button><el-button v-auth="'api/v1/system/menu/update'" @click="onOpenEditMenu(row)">修改</el-button><el-button v-auth="'api/v1/system/menu/delete'" type="danger" @click="onTabelRowDel(row)">删除</el-button></div></template></MobileRecordList>
		</el-card>
