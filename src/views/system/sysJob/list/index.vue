<template>
	<div class="system-sysJob-container">
		<el-card shadow="hover">
			...
			</el-table></template><template #default="{ row }"><div class="mobile-record-card__header"><h3 class="mobile-record-card__title">{{ row.jobName }}</h3><el-tag :type="row.status === 0 || row.status === '0' ? 'success' : 'info'">{{ row.status === 0 || row.status === '0' ? '启用' : '暂停' }}</el-tag></div><dl class="mobile-record-card__fields"><div><dt>任务组</dt><dd>{{ jobGroupFormat(row) }}</dd></div><div><dt>任务方法</dt><dd>{{ row.invokeTarget }}</dd></div><div><dt>CRON</dt><dd>{{ row.cronExpression }}</dd></div><div><dt>执行策略</dt><dd>{{ misfirePolicyFormat(row) }}</dd></div></dl><div class="mobile-record-card__actions"><el-button v-auth="'api/v1/system/sysJob/view'" type="primary" @click="handleView(row)">详情</el-button><el-button v-auth="'api/v1/system/sysJob/edit'" @click="handleUpdate(row)">修改</el-button><el-dropdown><el-button>更多</el-button><template #dropdown><el-dropdown-menu><el-dropdown-item><el-button v-auth="'api/v1/system/sysJob/run'" text @click="handleJobRun(row)">执行一次</el-button></el-dropdown-item><el-dropdown-item><el-button v-auth="'api/v1/system/sysJob/delete'" text type="danger" @click="handleDelete(row)">删除</el-button></el-dropdown-item></el-dropdown-menu></template></el-dropdown></div></template></MobileRecordList>
			<pagination
