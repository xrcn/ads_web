	...
	<div class="system-role-container">
        ...
        <el-form :inline="true">
          ...
          <el-form-item>
			...
			<MobileRecordList :data="tableData.data" data-mobile-view="system-role"><template #desktop><el-table :data="tableData.data" style="width: 100%"
        row-key="id"
        default-expand-all
        ...
        <el-table-column prop="createdAt" label="创建时间" show-overflow-tooltip></el-table-column>
			...
			</el-table></template><template #default="{ row }"><div class="mobile-record-card__header"><h3 class="mobile-record-card__title">{{ row.name }}</h3><el-tag :type="row.status === 1 ? 'success' : 'info'">{{ row.status === 1 ? '启用' : '禁用' }}</el-tag></div><dl class="mobile-record-card__fields"><div><dt>排序</dt><dd>{{ row.listOrder }}</dd></div><div><dt>用户数量</dt><dd><el-link type="primary" @click="onOpenUserList(row)">{{ row.userCnt }}</el-link></dd></div><div><dt>角色描述</dt><dd>{{ row.remark || '-' }}</dd></div></dl><details class="mobile-record-card__details"><summary>查看完整信息</summary><dl class="mobile-record-card__fields"><div><dt>创建时间</dt><dd>{{ row.createdAt || '-' }}</dd></div></dl></details><div class="mobile-record-card__actions"><el-button type="primary" @click="onOpenEditRole(row)">修改</el-button><el-dropdown @command="handleCommand"><el-button>授权</el-button><template #dropdown><el-dropdown-menu><el-dropdown-item :command="'resource_'+row.id">资源权限</el-dropdown-item><el-dropdown-item :command="'data_'+row.id">数据权限</el-dropdown-item><el-dropdown-item :command="'user_'+row.id">用户授权</el-dropdown-item></el-dropdown-menu></template></el-dropdown><el-button type="danger" @click="onRowDel(row)">删除</el-button></div></template></MobileRecordList>
		</el-card>
