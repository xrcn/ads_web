...
- 全部前端闭包完成后，用户已授权按实时生产预检结果发布并统一验收；该授权不覆盖数据库、后端、账号、权限、域名、Nginx 或基础设施变更。
...
- 只读检查：`E:/ads/发布develop到生产.ps1` 及其当前依赖配置。
- 不修改发布脚本、数据库、后端或基础设施配置。

**门禁：**

- [ ] **步骤 1：实时只读预检**

确认 `ads_web` 主工作区处于 `develop`，候选 commit 已记录，工作树干净，生产服务器身份与 `known_hosts` 匹配，远端前端目录和可用备份/rollback 路径可验证。

- [ ] **步骤 2：候选构建与哈希**

在主工作区运行已通过的最终 `npm run build`，记录 `dist` 产物哈希；确认发布脚本只会部署该候选前端产物，不触发数据库、后端、缓存、Nginx 或 systemd 写操作。

- [ ] **步骤 3：发布与健康检查**

仅在步骤 1 和步骤 2 全部通过后，执行当前预检确认的专用前端发布命令。记录远端文件哈希、HTTP 健康检查、部署版本和 rollback 标识。

- [ ] **步骤 4：生产统一验收**

在生产 `admin.xrc666.com` 的真实登录态中，以 390×844 和 430×932 验收设计文档第 5 节全部页面；记录通过、失败和阻塞项。任何核心页面仍有裁切、空白、权限偏差或发布后健康检查失败时，立即执行已验证 rollback 并停止扩大操作。

- [ ] **步骤 5：发布完成报告**

报告候选 commit、生产目标、发布命令、哈希、健康检查、统一验收矩阵、rollback 状态、未解决风险和全部实际文件改动。
